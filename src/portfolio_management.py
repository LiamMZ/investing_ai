...
import numpy as np
from portfolio_mgmt_ai import PortfolioManagementModel
import pandas as pd
import time
...
class PortfolioManagementSystem(TradingSystem):
    ...
    def __init__(self):
        super().__init__(AlpacaPaperSocket(), 'IBM', 6, 1, 'AI_PM')
...
data_req = AlpacaPaperSocket().get_barset('IBM', timeframe='1D', limit=1).df
            # Construct dataframe to predict
x = pd.DataFrame(
    data=[[
        data_req['IBM']['close'][0]]], columns='Close'.split()
